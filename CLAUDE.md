...
- **DSH 0.1.5 breaks two shell assumptions (fixed 2026-09):** (1) `dsh web` now auto-opens the system default browser — the spawn contract must pass `--no-open`. (2) The banner URL carries `?token=…` and the web fence rejects bare `http://127.0.0.1:<port>` with "dsh web authentication required" — the window must `loadURL` the full banner URL (supervisor exposes `onReadyUrl` via `parseBannerUrl`; API auth is cookie-based, established by that first tokened load). (3) **The old notify API is gone**: `/api/events.mux`, `/api/events.host` and `POST /api/host.describe` no longer exist; events moved to a Typert RPC gateway at `/api/remote.mux` (upgrade-gated, `approval/asked`+`approval/decided` frames) — until the notify-bridge is re-ported, approval/turn notifications are silently absent (bridge/probe fail soft) and describe-probe logs a 405 warn.
